...
## 🚀 最近升级概要（M1-M4 已上线 · M5 交互升级进行中 · v0.9.0）

- **M5 交互升级（v0.9.0）**：针对三个使用痛点重做前端 ——
  ① **历史回放命中可视化**：命中红/蓝球金色描边 + ✓ 角标一眼可辨，每注带奖级徽标与「红中 X/6」摘要，
  期号下拉直接标注每期最好成绩（红中数 / 蓝球 / 奖级），顶部汇总最好奖级 / 平均红中 / 蓝球命中 / 总奖金，支持按命中排序；
  ② **数据分析增强**：红球热力图三模式（频率 / 当前遗漏 / 遗漏比）、蓝球 16 格面板 + 近 30 期轨迹、
  和值 / 三区比 / 跨度 / 奇偶四类走势图（均值 ±σ 与分位线标注）、和值 / 跨度 / 奇偶 / 三区比 / AC 值 / 大小 / 012 路 / 质数
  八类分布直方图（结构指标叠加超几何理论值对照）、当前遗漏 vs 平均遗漏散点与遗漏区间分组、号码详情弹窗新增近 100 期出现轨迹；
  ③ **评估报告重组**：单卡片堆叠改为「综合概览 / 累计对照 / 方法对比 / ML 模型 / LLM 通道」五个子页，
  离线评估新增系统 vs 随机 KPI 对比卡与奖级分布图，累计报表新增方法切换、95% CI 列与 10 期滚动趋势图（含 CI 带）。
...
单页中文仪表盘（dark 主题，ECharts，Tab 工作台）：

- **预测中心**：Top-10 候选注（红/蓝球圆点、置信度条、方法徽标、LLM 推理理由、复制/收藏）、「我的号码诊断」；
- **数据分析**：多尺度概览指标卡、33 格红球热力（频率 / 当前遗漏 / 遗漏比三模式）、蓝球 16 格面板 + 近 30 期轨迹、
  和值 / 三区比 / 跨度 / 奇偶走势图、八类分布直方图（奇偶 / 大小 / 012 路 / 质数叠加超几何理论值）、
  遗漏散点（当前 vs 平均）与遗漏区间分组、号码点选详情（含近 100 期出现轨迹）；
- **规律研究**：全部规律的回测表格（样本量/期望/边际/p 值/adj_p/等级）、红牌预警、多选边际对比、自动挖掘；
- **历史回放**：开奖走势矩阵、历史预测回放（命中球金框 ✓ 高亮、奖级徽标、期号下拉带命中摘要、按命中排序）、系统回放诊断；
- **评估报告**：五个子页 —— 综合概览（系统 vs 随机 KPI 对比卡、红球命中/奖级分布图、在线对照记录）、
  累计对照（按方法聚合 + 95% CI + 10 期滚动趋势图）、方法对比（paired 筛查建议）、ML 模型（Brier/校准/paired）、
  LLM 通道（三通道对比与成本）。
...
- **历史回放命中可视化**：命中红/蓝球金框 + ✓ 角标；每注票卡带奖级徽标（如「五等奖 ¥10」）与「红中 X/6 · 蓝 ✓」摘要；
  期号下拉直接标注每期最好成绩；顶部新增最好奖级 / 平均红中 / 蓝球命中 / 中奖注数 / 总奖金汇总卡；支持按命中排序
- **数据分析增强**：红球热力图三模式（频率 / 当前遗漏 / 遗漏比，上期开出号码虚线框标记）；蓝球 16 格面板（频率 / 遗漏）
  与近 30 期轨迹；和值 / 三区比 / 跨度 / 奇数个数四类走势图（均值、±1σ、P5/P95 标注）；和值 / 跨度 / 奇偶 / 三区比 /
  AC 值 / 大小 / 012 路 / 质数八类分布直方图，其中结构类指标叠加**超几何理论值**对照（诚实展示"结构与随机一致"）；
  当前遗漏 vs 平均遗漏散点图（对角线参考，偏冷/偏热着色）与遗漏区间分组；号码详情弹窗新增近 100 期出现轨迹图
- **评估报告重组**：拆分为「综合概览 / 累计对照 / 方法对比 / ML 模型 / LLM 通道」五个子页，操作按钮按子页归属拆分并附说明；
  离线评估新增系统 vs 随机 KPI 对比卡（Δ 着色）与奖级分布图（后端 `aggregate` 新增 `levels_dist`）；
  累计报表新增方法下拉切换、95% CI 列（最近 30 期滚动窗口）与红球命中 / 蓝球命中率 10 期滚动趋势图（含 CI 带）；
  在线对照记录独立展示并按新→旧排序；方法建议状态着色徽标
- **其它**：号码详情弹窗提升为全局组件（预测中心点击号码也可查看）；`生成预测 / 重新回测 / 自动挖掘` 等按钮补齐 busy 状态；
  版本号 v0.8.3 → v0.9.0（build `2026-09-M5.1`）
